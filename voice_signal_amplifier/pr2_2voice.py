...
# python pr2_2voice.py domisoldo.wav 2nd_recording.wav 8000 6067
...
stf_1 = librosa.stft(voice_1_, n_fft=n_fft, hop_length=hop_length)
stf_2 = librosa.stft(voice_2_, n_fft=n_fft, hop_length=hop_length)
...
print('len(log_spectrogram_1),len(log_spectrogram_2), = ',len(log_spectrogram_1),len(log_spectrogram_2))
print('len(log_spectrogram_1),len(log_spectrogram_2), = ',log_spectrogram_1.shape,log_spectrogram_2.shape)
#hz_range = [0, 8000]
...
dol_start_time=1.0; dol_end_time=2.0; mi_start_time=2.3; mi_end_time=3.3; 
sol_start_time=3.6; sol_end_time=4.6; doh_start_time=5.0; doh_end_time=6.0;
dol_start_index=int(dol_start_time*sr/hop_length); dol_end_index=int(dol_end_time*sr/hop_length);
mi_start_index=int(mi_start_time*sr/hop_length); mi_end_index=int(mi_end_time*sr/hop_length);
sol_start_index=int(sol_start_time*sr/hop_length); sol_end_index=int(sol_end_time*sr/hop_length);
doh_start_index=int(doh_start_time*sr/hop_length); doh_end_index=int(doh_end_time*sr/hop_length);
Nf = len(log_spectrogram_1); # n_fft/2
dol_1_spectrum=np.zeros(Nf,dtype='float'); mi_1_spectrum=np.zeros(Nf,dtype='float');
sol_1_spectrum=np.zeros(Nf,dtype='float'); doh_1_spectrum=np.zeros(Nf,dtype='float');
dol_2_spectrum=np.zeros(Nf,dtype='float'); mi_2_spectrum=np.zeros(Nf,dtype='float');
sol_2_spectrum=np.zeros(Nf,dtype='float'); doh_2_spectrum=np.zeros(Nf,dtype='float');
for i in range(dol_start_index,dol_end_index): 
    dol_1_spectrum += log_spectrogram_1[:,i]; dol_2_spectrum += log_spectrogram_2[:,i];
...
for i in range(mi_start_index,mi_end_index): 
    mi_1_spectrum += log_spectrogram_1[:,i]; mi_2_spectrum += log_spectrogram_2[:,i];
...
for i in range(sol_start_index,sol_end_index): 
    sol_1_spectrum += log_spectrogram_1[:,i]; sol_2_spectrum += log_spectrogram_2[:,i];
sol_1_spectrum /= (sol_end_index-sol_start_index); sol_2_spectrum /= (sol_end_index-sol_start_index); 
for i in range(doh_start_index,doh_end_index): 
    doh_1_spectrum += log_spectrogram_1[:,i]; doh_2_spectrum += log_spectrogram_2[:,i];
doh_1_spectrum /= (doh_end_index-doh_start_index); doh_2_spectrum /= (doh_end_index-doh_start_index); 
#
fbin=np.linspace(0,sr/2,Nf);
plt.figure(4); plt.plot(fbin,dol_1_spectrum,'b'); plt.plot(fbin,dol_2_spectrum,'r'); 
plt.title('average spectrum of DO low: Original(BLUE) 2nd Recording(RED)'); plt.show()
plt.figure(5); plt.plot(fbin,mi_1_spectrum,'b'); plt.plot(fbin,mi_2_spectrum,'r'); 
plt.title('average spectrum of MI: Original(BLUE) 2nd Recording(RED)'); plt.show()
plt.figure(6); plt.plot(fbin,sol_1_spectrum,'b'); plt.plot(fbin,sol_2_spectrum,'r'); 
plt.title('average spectrum of SOL: Original(BLUE) 2nd Recording(RED)'); plt.show()
plt.figure(7); plt.plot(fbin,doh_1_spectrum,'b'); plt.plot(fbin,doh_2_spectrum,'r'); 
plt.title('average spectrum of DO high: Original(BLUE) 2nd Recording(RED)'); plt.show()
