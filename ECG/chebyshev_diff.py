...
def plot_fft2(y,SAMPLE_RATE):
   ...
   fbins = np.arange(len(fft_y)) * (SAMPLE_RATE/2 / len(fft_y)) ; 
   plt.subplot(2,1,2); plt.loglog(fbins, fft_y);
   plt.title('L frequency spectrum / blue: before, orange: after Chebyshev LPF');

f= open('inter_L.txt', 'r');
...
plt.subplot(2,1,1); plt.plot(filtered_out_waveform); plt.title('L: blue: before, orange: after Chebyshev LPF');
